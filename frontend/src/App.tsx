...
if (isSafeMode && !(window as any).pywebview) {
  ...
  evtSource.addEventListener("config_update", () => {
    if ((window as any).onConfigUpdated) (window as any).onConfigUpdated();
  });
  evtSource.addEventListener("biome_update", (e) => {
    if ((window as any).onBiomeUpdate) (window as any).onBiomeUpdate(JSON.parse(e.data));
  });
  evtSource.addEventListener("update_available", (e) => {
    const info = JSON.parse(e.data);
    if ((window as any).onUpdateAvailable && info) (window as any).onUpdateAvailable(info.version, info.url);
  });
  evtSource.addEventListener("update_status", (e) => {
    if ((window as any).onUpdateStatus) (window as any).onUpdateStatus(JSON.parse(e.data));
  });
  evtSource.addEventListener("macro_status", (e) => {
    if ((window as any).onMacroStatus) (window as any).onMacroStatus(JSON.parse(e.data));
  });
  evtSource.addEventListener("shortcut", (e) => {
    if ((window as any).onShortcutEvent) (window as any).onShortcutEvent(JSON.parse(e.data));
  });
}
