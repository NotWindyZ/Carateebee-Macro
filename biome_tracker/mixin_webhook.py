...
class WebhookMixin:
    def send_screen_screenshot_webhook(self, screenshot_path):
        try:
            ...
            if not urls:
                return
            content = ""
            icon_url = "https://i.postimg.cc/TYwcDvtG/mambo.png"
            ...
            embed = {
                ...
                "color": 0xffffff,
                "footer": {"text": f"Carateebee Mambooo {current_ver}", "icon_url": icon_url},
    ...
    def send_webhook(self, biome, message_type, event_type, screenshot_path=None):
        ...
        embed = {
            ...
            "color": biome_color,
            "footer": {
                "text": f"""Carateebee Mambooo {current_ver}""",
                "icon_url": icon_url
            },
            "timestamp": current_utc_time
        }
    ...
    def send_webhook_status(self, status, color=None):
        try:
            ...
            if not urls:
                self.append_log("Webhook URL is missing/not included in the config")
                return
            is_start = "started" in status.lower()
            default_color = 3066993 if is_start else 15158332
            embed_color = color if color is not None else default_color
            icon_url = "https://i.postimg.cc/TYwcDvtG/mambo.png"
            if is_start:
                n = len(urls)
                status = f"Let get into training Trainer >:D!! ({n} webhook{'s' if n != 1 else ''} active)"
            elif "stopped" in status.lower() or "stopping" in status.lower() or "failed" in status.lower() or "restarted" in status.lower() or "reconnect" in status.lower() or "traceback" in status.lower() or "error" in status.lower() or "max retries" in status.lower():
                status = "That was a great training session torena!"
            else:
    ...
    def send_macro_summary(self, last24h_seconds, reason_text="That was a great training session torena teehee ~!"):
        try:
